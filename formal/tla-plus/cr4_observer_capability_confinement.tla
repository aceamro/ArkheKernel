...
(*
 * cr4_observer_capability_confinement — DIP-N5 sub-step E.7.
 *
 * CR-4 anchors E15 (Observer Capability Confinement, chain-non-
 * affecting, 4-clause). Phase 2 closing module — together with
 * CR-1 (Adversary A E14) + CR-2 (E4-E7 multi-shell) + CR-3 (E11-
 * E13 + PQC) + R4-I (E3+E8+E9 layering/DAG), CR-4 closes the v0.12
 * sealing chain at the formal-method level.
 *
 * EXTENDs runtime_core with:
 *  (1) ObserverCapToken set ({"PgWrite"} v0.12, additive enum);
 *  (2) ObserverInvocation record (observer + cap_token + payload);
 *  (3) ObserverPanic + ObserverQuarantine event records;
 *  (4) 3 module-specific INVs + 1 theorem + 1 lemma;
 ...
 * uncontrolled syscall egress. E15.a closes native-crash channel;
 * E15.b closes uncontrolled egress. Residual: (i) host-call API
 * implementation defects (cryptographer + veteran scope per Track
 * A.2 chain-non-affecting 4-clause invariant) + (ii) wasmtime
 * engine zero-day (out-of-scope per `docs/implementation-plan.md`
 * §19, symmetric with E14.L2 wasmtime zero-day exclusion).
 *
 ...
 *
 * CR-1 + CR-3 + CR-4 closes v0.12 sealing chain at formal-method
 * level: chain integrity (compute) + chain anchoring (policy) +
 ...
 *
 * Cryptographer LEAD primary verify scope per cycle plan v0.4 (3rd
 * activation): Adversary B residual closure proof primary + 4-clause
 * coverage + panic close mechanism + temporal semantic check axis
 * (process maturity carry from E.5 cryptographer self-observation).
 *
 * theorist Minor Note 1 absorption: NextCR4 self-contained pattern
 * (5-cycle pattern firmly established at CR-1+CR-2+CR-3+R4-I+CR-4 —
 * E.7 close decision retains self-contained convention).
 * theorist Minor Note 2 absorption: TypeOK_CR4 explicit composition
 * via `TypeOK /\ ...` (4-cycle pattern: E.4+E.5+E.6+E.7 firmly
 * established).
 *
 * Anchored to:
...
CONSTANTS
    ...
    \* @type: Set($capToken);
    ObserverCapTokens,       \* {"PgWrite"} v0.12, additive enum
    \* @type: Int;
...
ASSUME
    /\ ObserverIds # {}
    /\ Payloads # {}
    /\ ObserverCapTokens = {"PgWrite"}        \* v0.12 fixed set
 ...
 *     E.4+E.5+E.6 carry-forward confirmed at E.7 / 4-cycle pattern
 *     firmly established) --- *)
...
\* from the allowed ObserverCapTokens set. v0.12 = {"PgWrite"}.
...
\* INV E15-4: CapTokenSealed (MC, M2.4 sealed-trait safeguard, DIP-N6
\* Phase 2). Type-system anchor of E15.b capability confinement —
\* every observer/hook cap_token is drawn from a *sealed* universe
\* of types (Rust `pub trait HookCapTokenSealed: private_seal::Sealed`
...
\* finite set (declared at line 101 ASSUMEd to `{"PgWrite"}` v0.12).
\* The CapTokenSealed INV at TLA+ level expresses that every observed
\* cap_token in the model is drawn from this CONSTANTS-fixed set —
\* the same property `ObserverCapabilityConfined` already enforces.
\* The sealed-trait safeguard adds a *type-system layer* of enforcement
\* on the Rust side: pre-M2.4 the trait was open, allowing external
\* impls to weaken the cap_token universe at compile time; post-M2.4
\* the trait is sealed, so the Rust-side universe matches the TLA+
\* abstraction faithfully.
\*
...
\* typecheck capture the M2.4 strengthening event.
\*
\* Anchored to:
 ...
 *   E15-CapTokenSealed               (MC, M2.4 sealed-trait safeguard
 ...
 *   SealedTrait_implies_E15.b        (lemma, M2.4 sealed-trait
 *                                    derives E15.b confinement at
 *                                    type-system level; see comment
 *                                    block below)
 *)

(* --- Adversary B residual reduction lemma (cryptographer LEAD) ---
 *
 ...
 *
 * E15 closure (Track A.2 chain-non-affecting 4-clause invariant):
 *
 ...
 *   #[non_exhaustive] enum (v0.12 = {PgWrite}); module-load
 ...
 *
 *   (i)  Host-call API implementation defects — covered by Track
 *        A.2 chain-non-affecting 4-clause invariant verification
 *        (cryptographer + veteran scope per implementation).
 *        Residual implementation defects are out-of-scope at the
 *        TLA+ refinement level; addressed by Kani 4-property suite
 *        at E.8 (memory bounds-check property) + integration tests
 *        in arkhe-forge-platform/src/observer_host/.
 *
 ...
 *     compute determinism). Together with Adversary B (CR-4),
 *     v0.12 sealing chain closes both chain-affecting and
 *     chain-non-affecting attack axes at the formal-method level.
 *
 *   - PQC downgrade adversary residual reduction (CR-3, E13
 *     chain-anchored policy). Together with Adversary A + B,
 *     v0.12 sealing chain closes compute + policy + observer axes.
 *
 * Process maturity carry (cryptographer E.5 self-observation):
 * E.7 LEAD primary verify scope explicitly includes temporal
 * semantic check axis. CR-4 has temporal semantic at panic-
 * quarantine ordering: panic_tick precedes emit_tick (host emits
 * receipt at current tick when panic occurred at p.panic_tick).
 * The 1:1 correspondence INV is tick-aware via panic_tick field
 * matching (no temporal gap allowed between panic and quarantine
 * emission).
 *)

(* --- SealedTrait_implies_E15.b lemma (M2.4 strengthening) ---
 *
...
(* --- theorist Minor Notes carry status (E.7 close decisions) ---
 *
 * Note 1 (base Next abstract operator vs CR-X self-contained):
 *   5-cycle pattern firmly established (CR-1+CR-2+CR-3+R4-I+CR-4
 *   all self-contained). E.7 close decision: RETAIN self-contained
 *   convention. Refactor of base runtime_core's Next abstract
 *   operator to a Next == NextCR1 ∨ NextCR2 ∨ ... composition
 *   would couple modules tightly; current approach maintains
 *   module independence for separate Apalache typecheck. Defer
 *   base-Next-refactor discussion to E.9 cycle close + DIP-N6
 *   follow-on (if any).
 *
 * Note 2 (TypeOK explicit composition):
 *   4-cycle pattern firmly established (E.4+E.5+E.6+E.7 all use
 *   `TypeOK_<MOD> == TypeOK ∧ ...`). Pattern is now the convention.
 *
 * Note 3 (ChainHashFn↔BLAKE3 connection comment in CR-1):
 *   CR-4 not affected (CR-1 anchor). E.7 close decision: defer
 *   to E.9 cycle close (1-line comment addition in CR-1, low
 *   priority). Architect recommendation: add at E.9 + axiom
 *   evidence summary commit.
 *
 * Note 4 (AuthenticateActor user_id strengthening):
 *   CR-4 not affected (CR-2 anchor). E.7 close evaluation: no
 *   action needed at v0.12 — current CR-2 formulation satisfies
 *   E5+E6 axiom coverage. Defer further strengthening to v0.13+
 *   if needed.
 *
 * Note 5 (ShellIsolationOnReplay tier annotation):
 *   CR-4 not affected (CR-2 anchor). E.7 close evaluation: tier
 *   annotation accurate per current spec body (§11.2 E7 dual-tier
 *   TP submit + RA replay). No revision needed.
 *
 * R4-I Space coverage decision (theorist Minor Note 5 evaluation
 * candidate at E.6): SOUND — Entry-only modeling at TLA+ abstraction
 * is sufficient via structural isomorphism argument. Documented
 * at r4 module-level comment lines 33-41.
 *)
