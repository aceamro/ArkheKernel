...
### 1. Project release tag (`v0.11`)

- **Location**: release marker across the repo. Each crate's `Cargo.toml` `[package] version` (`0.11.0`), git tag (`v0.11`), `CHANGELOG.md` entry.
...
- **Policy**: asymptotic integer bump (0.11 → 0.12 → … → 0.99 → 0.100). 1.0 is intentionally never reached. This project aims for a single final release at v0.11, and even if fixes are later introduced, the same version is retained.

### 2. L0 kernel ABI snapshot (`(0, 10)` / `(0, 10, 0)`)

- **Location**: `arkhe-kernel/src/persist/wal.rs`
  ```rust
  pub const CURRENT_KERNEL_SEMVER: (u16, u16, u16) = (0, 10, 0);
  pub const ABI_VERSION:           (u16, u16)      = (0, 10);
  ```
- **Audience**: binary compatibility verifiers of WAL / snapshot / event bytes (the replay path, external verifiers).
- **Meaning**: a marker indicating which snapshot the **serialized ABI** of the L0 kernel corresponds to. `(0, 10)` means "L0 ABI snapshot 0.10" — it identifies the set of byte formats including the WAL header, record layout, and `StepStage` discriminant.
...
This project includes L0 ABI snapshot `(0, 10)` in the `v0.11` release. The reasons the two values differ are as follows.
...
- **Different bump conditions**: the project release may be bumped frequently per unit of public change, but the L0 ABI is bumped only when there is a WAL / snapshot breaking change. In practice, L0 has had no ABI breaking change since v0.10, so `(0, 10)` is kept as is.
- **L0 DO NOT TOUCH protection**: the L0 kernel source is protected by baseline hashing (`ci/l0-baseline-hashes.txt`) so that it is not modified without re-review. If L0 source is modified on every project release bump, this protection becomes meaningless.

As a result, the two coordinate systems are independent, and the state in which project `v0.11` contains L0 ABI `(0, 10)` is the official combination for v0.11.
...
## Policy for future L0 ABI bump timing
...
- `v0.11` is the repo release, `(0, 10)` is the L0 ABI snapshot — the two are different axes.
- Even when the values differ, this is an intended state, and even if the project release is bumped, the L0 ABI is maintained independently.
...
- Within the v0.11 frozen window `Cargo.lock` pins the `toml` minor version, so the emitted bytes are stable.
...
**Drift response procedure** (architect Option C, 2026-04-25):
...
L0's WAL chain BLAKE3 hash is bound to §1's ABI snapshot `(0, 10)`. The WAL record `postcard` field order is DO NOT TOUCH #8 — Runtime-side changes can never disturb it.
